...
from src.aliyun_qwen_batch import read_jsonl
...
class AliyunQwenRealtimeOfflineTests(unittest.TestCase):
    ...
    def test_unexpected_future_exception_stops_pending_calls_and_finalizes_state(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            total = len(read_jsonl(source_dir / batch_cli.REQUEST_FILENAME))
            ...
            self.assertIn(failures[0]["custom_id"], {
                row["custom_id"]
                for row in read_jsonl(source_dir / batch_cli.REQUEST_FILENAME)
            })
