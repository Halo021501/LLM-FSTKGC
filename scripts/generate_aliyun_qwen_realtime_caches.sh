...
if [[ ! -f "$ENV_FILE" && -f .env.aliyun_qwen_batch ]]; then
  ENV_FILE=.env.aliyun_qwen_batch
fi
...
SOURCE_ROOT="${SOURCE_ROOT:-runs/aliyun_qwen_batch/${MODEL}/standard/${SCOPE}}"
...
Set REALTIME_SCOPE=smoke|pilot|full; the matching reviewed Batch preparation is
used only as immutable target-blind request/index input.
Set REALTIME_SPLIT=test|valid to choose the evaluated split. The valid split
reads the reviewed valid_s* Batch plans, writes valid_s* run directories and
