...
def prepare(args: argparse.Namespace) -> dict[str, object]:
    ...
    # _prepare_rows is the reviewed target-blind builder used by the realtime
    # path. It performs no network or credential read.
    request_rows, index_rows, base_plan = plan_cli._prepare_rows(args)
    ...
    for shard_id in range(args.num_shards):
        ...
        request_path = shard_dir / plan_cli.REQUEST_FILENAME
        index_path = shard_dir / plan_cli.INDEX_FILENAME
        plan_path = shard_dir / plan_cli.PLAN_FILENAME
        write_bytes_atomic(request_path, request_bytes)
        write_bytes_atomic(index_path, index_bytes)
        write_json_atomic(plan_path, plan)
        validation = validate_qwen_requests(request_path)
        if validation["sha256"] != plan["request_sha256"]:
            raise RuntimeError(f"post-write request hash mismatch: {shard_dir}")
        verified, _, _ = plan_cli._verify_job(shard_dir)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--shot", type=int, choices=sorted(plan_cli.SUPPORTED_SHOTS), required=True
    )
    ...
    parser.add_argument("--model", default=plan_cli.DEFAULT_REALTIME_MODEL)
